...
//   1. The prompt -> response database lives in static_qa_data.js. Each entry's
//      example questions are embedded once with all-MiniLM-L6-v2 (the same model
//      the Python side uses, here as its ONNX twin "Xenova/all-MiniLM-L6-v2" via
//      transformers.js) and cached as unit vectors.
...
//   3. The response on that nearest entry is returned verbatim. If nothing is
//      close enough (similarity below MATCH_SIMILARITY_THRESHOLD), a generic
//      fallback line is returned instead.
//
...
let corpus = null;          // { vectors: number[][], responses: string[] }
...
export async function load(onProgress) {
  ...
  loadingPromise = pipeline("feature-extraction", EMBED_MODEL_ID, {
    ...
    .then(async (pipe) => {
      ...
      corpus = { vectors: out.tolist(), responses };
    })
    .catch((err) => {
...
// `visitCounts` is accepted for parity with the old keyword version (app.js
// passes its per-session counts) but retrieval is stateless; only the generic
// fallback cycle keeps a little state, under `_fallback`. Async because the
// first call loads the embedding model (a small ~25 MB download, then cached).
export async function getResponse(playerInput, visitCounts) {
  ...
  for (let i = 0; i < corpus.vectors.length; i++) {
    ...
    if (score > bestScore) {
      bestScore = score;
      bestResponse = corpus.responses[i];
    }
  }

  if (bestResponse === null || bestScore < MATCH_SIMILARITY_THRESHOLD) {
